# Create a dictionary called `person`, it has three properties, `name` which is a string,
# `age` which is an integer, `hobbies` which is a list of strings.
...
# Create a function called `change_age` which takes the `person` dictionary and a `number` as arguments, 
#this function changes the age in the dictionary to the `number`. The return value of this function is the updated dictionary.
...
# Create a function called `add_hobby` which takes the `person` dictionary and a `hobby` as arguments. This function adds the 
#hobby to the list of `hobbies` inside the person dictionary. The function will return the updated dictionary.
...
print(add_hobby(person, "swimming"))
...
#     for job , value in job:
#         if person == value
#         return person
#         else person['job']=engineer 
#         print(person)
